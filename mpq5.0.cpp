...
int main() {
	if (MessageBoxW(NULL, L"mpq5.0 has more new GDI effects and sounds, want to run it?", L"mpq5.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
		ExitProcess(0);
	}
	else if (MessageBoxW(NULL, L"Press yes to begin", L"mpq5.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
		ExitProcess(0);
	}
	Sleep(700);
